...
import aqper.side_project.aop_part4_chapter07.data.models.PhotoResponse
...
class MainActivity : AppCompatActivity() {
            ...
            .into(
                object : CustomTarget<Bitmap>(SIZE_ORIGINAL, SIZE_ORIGINAL) {
                    override fun onResourceReady(
                        ...
                        val wallpapaerManager = WallpaperManager.getInstance(this@MainActivity)
                        ...
                        if (wallpapaerManager.isWallpaperSupported
                            && Build.VERSION.SDK_INT >= Build.VERSION_CODES.N
                            && wallpapaerManager.isSetWallpaperAllowed) {
                            snackbar.setAction("배경 화면으로 저장") {
                                try {
                                    wallpapaerManager.setBitmap(resource)
                                } catch (e: Exception) {
    ...
    private fun saveBitmapToMediaStore(bitmap: Bitmap) {
        ...
        resolver.openOutputStream(imageUri).use { outputSteram ->
            bitmap.compress(Bitmap.CompressFormat.JPEG, 100, outputSteram)
        }
